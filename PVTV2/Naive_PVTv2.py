...
class TransferTrainer:
    ...
    def build_model( self, input_shape=(32,32,3),  num_classes=100, dropout=0.25,resolution=1.0):
        ...
        outputs = tf.keras.layers.Lambda(lambda x: tf.keras.backend.clip(x, 0.0000001, 99999.))(outputs)
    ...
    def train(self,burn_in_epochs, finetuning_epochs, opt_class, opt_kwargs, lr_scheduler,ckp, burnin_lr=1e-1, 
        ...
        sio.savemat('PVTv2_Naive_Results_CIFAR100_3.mat', mdict = {'Loss':Loss, 'ValLoss':ValLoss, 'Acc':Acc, 'ValAcc':ValAcc})
        self.model.save_weights('PVTv2_Naive_Weights_CIFAR100_3.h5')
...
def train(use_tpu=False):
    with graph.as_default():
        ...
        for trial in range(1):
            ...
            ckp = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join('./Checkpoint', model_name+'EfficientNetB0_weights.{epoch:02d}-{val_acc:.2f}.hdf5'),monitor='val_acc',save_weights_only=True,verbose=1)
